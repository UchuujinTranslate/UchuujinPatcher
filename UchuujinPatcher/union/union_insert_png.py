...
def insert_png(png):
    ...
    subfiles = \
        [bin_raw[i:i+subfile_size] for i in range(0, len(bin_raw), subfile_size)]

    gzip_sizes = \
        meta["gzip_sizes"][first_subfile_id:first_subfile_id+len(subfiles)]
    gzip_offsets = \
        meta["gzip_offsets"][first_subfile_id:first_subfile_id+len(subfiles)]

    for i in range(0, len(subfiles)):
        out = io.BytesIO()
        with gzip.GzipFile(fileobj=out, mode='w',
                        compresslevel=1, mtime=None) as fo:
            fo.write(bin_raw)
        compressed = out.getvalue()
        print(len(compressed))
        padded = compressed + b'\x00' * (gzip_sizes[i] - len(compressed))
        chunk_raw = chunk_raw[:gzip_offsets[i]] + padded + \
            chunk_raw[gzip_offsets[i] + len(padded):]
